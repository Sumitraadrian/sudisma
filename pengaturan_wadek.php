...
echo "Session User ID: $user_id, Role: " . $_SESSION['role'] . "<br>"; // Debug session

// Query to get Wakil Dekan's profile information
$query = "SELECT wakil_dekan.id, wakil_dekan.nama, wakil_dekan.email, wakil_dekan.image 
          FROM users 
          JOIN wakil_dekan ON users.wakil_dekan_id = wakil_dekan.id 
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    $nama_wadek = $row['nama'];
    $email_wadek = $row['email'];
    $current_image = $row['image'];
} else {
    echo "Data Wakil Dekan tidak ditemukan!";
    exit();
}

// Profil update process
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    $new_nama_wadek = mysqli_real_escape_string($conn, $_POST['nama']);
    $new_email_wadek = mysqli_real_escape_string($conn, $_POST['email']);

    $query_update_profile = "UPDATE wakil_dekan 
                             JOIN users ON wakil_dekan.id = users.wakil_dekan_id 
                             SET wakil_dekan.nama = '$new_nama_wadek', 
                                 wakil_dekan.email = '$new_email_wadek' 
                             WHERE users.id = '$user_id'";

    if (mysqli_query($conn, $query_update_profile)) {
        $_SESSION['status'] = "Profil berhasil diperbarui!";
        $_SESSION['status_type'] = 'success';
        header("Location: pengaturan_wadek.php");
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['profile_picture'])) {
    ...
    if ($file['size'] <= 1048576 && in_array($file['type'], ['image/jpeg', 'image/png'])) {
        if (move_uploaded_file($file['tmp_name'], $target_file)) {
            ...
            $query_update_image = "UPDATE wakil_dekan SET image = '$file_name' 
                                   WHERE id = (SELECT wakil_dekan_id FROM users WHERE id = '$user_id')";
            if (mysqli_query($conn, $query_update_image)) {
                $_SESSION['status'] = "Foto profil berhasil diunggah!";
                $_SESSION['status_type'] = 'success';
            } else {
            ...
            $_SESSION['status'] = "Gagal mengunggah gambar!";
            $_SESSION['status_type'] = 'danger';
        }
    } else {
        $_SESSION['status'] = "File tidak valid (pastikan file berformat .jpg atau .png dan ukurannya < 1MB)";
        $_SESSION['status_type'] = 'danger';
    }
}
...
if (isset($_SESSION['user_id'])) {
    ...
    $queryWadek = "SELECT wakil_dekan.nama AS namaWadek FROM wakil_dekan
                   JOIN users ON users.wakil_dekan_id = wakil_dekan.id 
                   WHERE users.id = ?";
    ...
    if ($stmt) {
        ...
        if ($resultWadek->num_rows > 0) {
            ...
            $namaWadek = "Unknown"; // Nama default jika tidak ditemukan
        }
...
<head>
    ...
    <title>Pengaturan Akun Wadek</title>
...
<div class="container padding-top 40px">
        <h2>Pengaturan Akun Wakil Dekan</h2>
                        ...
                        <div class="card p-3">
                            ...
                            <form method="POST">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="nama" name="nama" value="<?= htmlspecialchars($nama_wadek); ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="email">Email</label>
                                    <input type="email" class="form-control" id="email" name="email" value="<?= htmlspecialchars($email_wadek); ?>" required>
                                </div>
