...
if (isset($_POST['approve_kajur']) || isset($_POST['reject_kajur'])) {
    $status_kajur = isset($_POST['approve_kajur']) ? 'disetujui' : 'ditolak';
    $id = $_POST['id'];

    // Update Kajur's decision
    $query = "UPDATE pengajuan SET pengajuan.status = ?, tanggal_acc_ketua_jurusan = NOW() WHERE id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("si", $status_kajur, $id);
    $stmt->execute();

    // Redirect to show the next approval process for Wadek
    header("Location: persetujuanadmin.php?id=$id");
    exit();
}

// Cek apakah tombol "Setuju" diklik
if (isset($_POST['approve_wadek']) || isset($_POST['reject_wadek'])) {
    ...
    <form method="post" class="d-inline">
        ...
        <div class="d-flex gap-2">
            <?php if (empty($pengajuan['status'])): ?>
                <!-- Tombol persetujuan Kajur (Belum ada status Kajur) -->
                <button type="submit" name="approve_kajur" class="btn btn-success">Setujui (Kajur)</button>
                <button type="submit" name="reject_kajur" class="btn btn-danger">Tolak (Kajur)</button>
            <?php elseif ($pengajuan['status'] == 'disetujui' && empty($pengajuan['status_wadek'])): ?>
                <!-- Tombol persetujuan Wadek (Status Kajur sudah disetujui dan belum ada status Wadek) -->
                <button type="submit" name="approve_wadek" class="btn btn-success">Setujui (Wadek)</button>
                <button type="submit" name="reject_wadek" class="btn btn-danger">Tolak (Wadek)</button>
            <?php else: ?>
                <span class="text-muted">Aksi sudah selesai.</span>
            <?php endif; ?>
        </div>
    </form>
</p>
